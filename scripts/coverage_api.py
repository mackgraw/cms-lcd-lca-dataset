...
def _status_code(status: str | None) -> str:
    """
    Map human strings to CMS Reports status codes:
      Active -> 'A', Retired -> 'R', Future/Future Effective -> 'F'
      None/unknown -> 'all' (omit filter)
    """
    if not status:
        return "all"
...
def _pick_list(data: Any) -> List[dict]:
    """Return a list of rows from common top-level shapes."""
...
def _get(path: str, params: Optional[Dict[str, str]] = None, timeout: int = 30) -> Any:
    """
    GET with retries, plus debug dumps so we can inspect actual responses
    in CI: artifacts/debug_<endpoint>.json
    """
    params = params or {}
    url = f"{BASE_URL}{path}"
    ...
    # Write a lightweight debug file so you can inspect the exact response in Actions
    try:
        ...
        # Breadcrumb to job logs
        if isinstance(payload, dict):
            top_keys = list(payload.keys())[:6]
            print(f"[DEBUG] GET {full_url} -> {status}; keys={top_keys}")
...
def list_final_lcds(
    ...
    """
    CMS 'Final LCDs' report.
    Uses lcdStatus=A|R|F (omit when 'all'), not 'status=Active'.
    """
    params: Dict[str, str] = {}
    if states:
...
def list_articles(
    ...
    """
    CMS 'Local Coverage Articles' report.
    Uses articleStatus=A|R|F (omit when 'all').
    """
    params: Dict[str, str] = {}
    if states:
        params["state"] = ",".join(states)
